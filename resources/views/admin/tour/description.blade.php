<div class="form-group">
	...
	@if (!empty($cd))
		...
		<div class="col-md-12">
			...
			<div class="col-md-10">
	            <textarea name="td_content[]" class="form-control {{ isset($cd->cd_type) && $cd->cd_type == '2'? 'editor' : '' }}" id="editor{{ isset($cd->cd_id)? $cd->cd_id : '' }}" rows="3"></textarea>
			</div>
		</div>
	@else
		...
		<div class="col-md-12">
			...
			<div class="col-md-10">
	            <textarea name="td_content[]" class="form-control {{ isset($data->cateDesc->cd_type) && $data->cateDesc->cd_type == '2'? 'editor' : '' }}" id="editor{{ isset($data->cateDesc->cd_id)? $data->cateDesc->cd_id : '' }}" rows="5">{{ $data->td_content }}</textarea>
			</div>
		</div>
	@endif
</div>	